...
For each domain, pay attention to:

- **Module structure**: How modules are organized, use of `use`, `import`, `alias`
- **Behaviours**: Any custom behaviours or callbacks defined
- **Pattern matching**: How pattern matching is used in function heads
- **Pipelines**: Data transformation patterns using `|>`
- **Error handling**: Use of `{:ok, _}` / `{:error, _}` tuples, `with` statements
- **Documentation**: `@moduledoc`, `@doc`, typespecs (`@spec`)
- **Testing patterns**: How the domain is tested (unit, integration, LiveView tests)
...
- Include the file paths where patterns are found.

Your goal is to document how the "{domain}" domain is implemented within this specific Elixir LiveView codebase in such a way that anyone could leverage or add features to it following the same conventions.
